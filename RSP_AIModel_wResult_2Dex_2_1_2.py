...
def getBias(sHistory, L) -> list:
    ...
    for i in range(3):
        ...
        if bias[i] < biasLowerLimit:
            ...
            # If there is 0 in the bias value,
...
def tagPredictions(patWeight, history) -> list:
    ...
    while i < 3:
        ...
        for j in range (wCount):
            ...
            pred[i] += patWeight[i][j] * history[0][j]
    ...
    retPred = sortPredictions(pred, history[0])

    # Get soft value

    bias = getBias(history[1], N)
...
def normWeight(weight, history) -> None:

    # Get bias

    bias = getBias(history[1], N * 2)
    totalBias = sum(bias)
    ...
    for i in range(3):
        ...
        pred = tagPredictions(weight[i], history)
...
def predict(m, his, weight, preHand, state) -> int:
    ...
    if hasHistory:
        ...
        bias = getBias(his[1], N)
        loseBias = bias[lose] / sum(bias)
        ...
        for i in range(3):
            ...
            if prev[i] * cur[i] <= 0:
                ...
                for j in range(3):
                    ...
                    weight[prevState][i][j] += float(res[j] * his[0][j] * loseBias)

                # Update pattern weights

                for j in range(hisCount):

                    weight[prevState][i][j + 3] += float(prev[i] * his[0][j + 3] * loseBias)
    ...
    normWeight(weight, his)
    
    # Prediction

    handPred = tagPredictions(weight[state], his)
    ...
    for i in range(len(weight)):
        ...
        for j in range(len(weight[i])):
            ...
            for k in range(len(weight[i][j])):
                ...
                weight[i][j][k] = random.randint(-2, 2)
